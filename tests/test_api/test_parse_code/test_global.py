...
def test_parse_code_simple_code(
    ...
    assert api.parse_code(simple_code_1_error, logging_level) == [
        parsing.output_structure_constructor(None, 4, "b", "global", logging_level)
    ]

    assert api.parse_code(simple_code_2_errors, logging_level) == [
        parsing.output_structure_constructor(None, 3, "a", "global", logging_level),
        parsing.output_structure_constructor(None, 4, "b", "global", logging_level),
    ]
...
def test_parse_code_for_code(for_code_no_errors, for_code_1_error):
    ...
    assert api.parse_code(for_code_1_error, logging_level) == [
        parsing.output_structure_constructor(None, 5, "i", "global", logging_level)
    ]
...
def test_parse_code_for_dict_code(
    ...
    assert api.parse_code(for_dict_code_1_error, logging_level) == [
        parsing.output_structure_constructor(None, 11, "value", "global", logging_level)
    ]

    assert api.parse_code(for_dict_code_1_other_error, logging_level) == [
        parsing.output_structure_constructor(None, 11, "key", "global", logging_level)
    ]

    assert api.parse_code(for_dict_code_2_errors, logging_level) == [
        parsing.output_structure_constructor(None, 9, "key", "global", logging_level),
        parsing.output_structure_constructor(None, 9, "value", "global", logging_level),
    ]
